...
void CDragDropListCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (m_pDragImage)
	{
		...
		if((point.x != m_lastMousePoint.x) || (point.y != m_lastMousePoint.y)){
			// Mouse location has really changed, so we can process the message
...
void CDragDropListCtrl::DropItem()
{
	...
	if(m_nDropIndex>m_anDragIndexes[0])m_nDropIndex++;
