...
DWORD CAralProcessAPI::GetPrimaryThreadId(DWORD dwPID, DWORD dwADR)
{
	if(dwPID == 0) return 0;
	...
	do 
	{ 
		GetThreadStartAddress(te32.th32ThreadID, &dwTidAdr);
		if( dwTidAdr == dwADR && te32.th32OwnerProcessID == dwPID )
		{
...
DWORD CAralProcessAPI::GetPrimaryThreadIdNew(DWORD dwPID)
{
	if(dwPID == 0) return 0;
...
BOOL CAralProcessAPI::GetThreadStartAddress(ULONG tid, DWORD *address)  
{
	...
	if( /*IsBadReadPtr(addr, sizeof(DWORD)) || */(DWORD) addr == 0 )
		return FALSE;
	*address = (DWORD) addr;  
	return TRUE;  
}

DWORD CAralProcessAPI::GetPrimaryThreadIdHook(DWORD dwPID, DWORD dwTID)
{
	if(dwPID == 0) return 0;
	...
	do 
	{ 
		if( te32.th32ThreadID == dwTID && te32.th32OwnerProcessID == dwPID )
		{
			GetThreadStartAddress(te32.th32ThreadID, &dwTidAdr);
			break;
		}
	...
	do 
	{ 
		GetThreadStartAddress(te32.th32ThreadID, &dwTidCom);
		if( dwTidCom == dwTidAdr && te32.th32OwnerProcessID == dwPID )
		{
			dwTID = te32.th32ThreadID;
			break;
		}
...
DWORD CAralProcessAPI::GetPrimaryThreadIdNext(DWORD dwPID, DWORD dwTID)
{
	if(dwPID == 0) return 0;
