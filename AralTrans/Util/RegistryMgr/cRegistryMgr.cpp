...
//	◎반환값 : 위의 경로와 이름에 해당하는 레지스트리 데이터(DWORD 형식)
//---------------------------------------------------------------------------
size_t CRegistryMgr::RegReadBINARY(CString rpath, CString name, unsigned char* retValue)
{
	int strindex;
	HKEY hCategoryKey;
	CString temp;
	...
	lRet = RegOpenKeyEx(hCategoryKey, rpath, 0, KEY_READ, &hKey);
	if(lRet == ERROR_SUCCESS){		// 키를 여는데 성공했다면
		if (RegQueryValueEx(hKey, name, 0, &type, NULL, &size) == ERROR_SUCCESS)
		{
			if(name.IsEmpty() || name.Compare(_T("0"))==0)
				lRet = RegQueryValueEx(hKey, NULL, 0, &type, (LPBYTE)retValue, &size);
			else
				lRet = RegQueryValueEx(hKey, name, 0, &type, (LPBYTE)retValue, &size);
...
//	◎반환값 : 성공시 TRUE, 실패시 FALSE
//---------------------------------------------------------------------------
BOOL CRegistryMgr::RegWriteBINARY(CString rpath, CString name, unsigned char* value, size_t len)
{
	int strindex, count;
	CString temp = _T("");
	HKEY hCategoryKey;
	...
	if(lRet != ERROR_SUCCESS) return FALSE;
	if(name.IsEmpty() || name.Compare(_T("0"))==0)
		RegSetValueEx(hKey, NULL, 0, REG_DWORD, (BYTE*)value, len);
	else
		RegSetValueEx(hKey, name, 0, REG_DWORD, (BYTE*)value, len);
